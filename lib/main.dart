...
void main() => runApp(const SmartChargeApp());
...
class _DashboardState extends State<Dashboard> with TickerProviderStateMixin {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(icon: const Icon(Icons.analytics, color: Colors.cyanAccent, size: 20), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (c) => LogView(logs: chargeLogs, costPerKwh: energyCost)))),
        ],
      ),
    );
  }
  ...
  Future<void> _addLogEntry(double kwh) async {
    final prefs = await SharedPreferences.getInstance();
    final newLog = {'id': DateTime.now().millisecondsSinceEpoch, 'date': DateTime.now().toIso8601String(), 'kwh': kwh, 'cost': kwh * energyCost};
...
class LogView extends StatefulWidget {
  ...
  const LogView({super.key, required this.logs, required this.costPerKwh});
  @override
